...
const ALL_TAG_LABELS = BLOCKS.flatMap((b) => b.tags);
...
function SkillCardTitle({ block, isDark }: { block: Block; isDark: boolean }) {
  const base =
...
export type SkillsTagCloudProps = {
  ...
  /** Matches main section titles (Summary, Experience, …) */
  sectionTitleClass?: string;
};

export function SkillsTagCloud({ isDark, headingClass, subheadingClass, sectionTitleClass }: SkillsTagCloudProps) {
  ...
  useEffect(() => {
    ...
    const paint = (clientX: number, clientY: number) => {
      ...
      const falloff = 96;
      for (const label of ALL_TAG_LABELS) {
  ...
  return (
    <div className="space-y-6 md:space-y-7">
      <div>
        ...
        >
          Three sticky disciplines. Notes lift on hover; tags pick up a little ink.
        </p>
      </div>
